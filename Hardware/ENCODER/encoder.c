...
void Encoder_TIM2_Init(void)
{
	...
	GPIO_InitStruct.GPIO_Mode = GPIO_Mode_IN_FLOATING;//CH1是A相，CH2是B相
...
int Read_Spead(int TIMx)
{
	...
	switch(TIMx)
	{
		...
		default:value_1 = 0;//若TIMx不是 2 或 4（即不支持的定时器），则 value_1赋值为 0。
	}
	return value_1;
}
...
void TIM2_IRQHandler(void)// TIM2中断服务函数
{
	...
	{
		TIM_ClearITPendingBit(TIM2, TIM_IT_Update);// 清除标志位（必须先做）
	}
}
